...
\begin{document}
  ...
  \end{titlepage}
  \begin{abstract}
    In den vergangenen Jahren ist aus dem World Wide Web mehr geworden als eine
    blosse Ansammlung statischer Dokumente. Es wir heute als \emph{sozialer
    Raum} wahrgenommen in dem die \emph{Netzbewohner} virtuelle \emph{Orte}
    aufbauen, sich gegenseitig \emph{besuchen} und wo sie an anderen sozialen
    Aktivit\"aten teilnehmen.
    
    Es kann angenommen werden, da\"s die Gastgeber (hosts) an diesen Pl\"atzen
    ein grosses Interesse daran h"atten, die Aktivit\"at in ihrem
    Online-\emph{Territorium} zu beobachten - wenn es denn eine einfache
    M"oglichkeit dazu g"abe.
    
    Obwohl Informationen \"uber Besucher fast \"uberall in der Form von
    Webserver-Logs bereitliegen beschr"anken sich die kommerziell erh"altlichen
    Auswertungstools (z.B. AWStats\cite{awstats}) auf die blosse Darstellung
    statistischer Informationen und erforschen nicht den umfangreichen Kontext,
    in dem die Besuche stattfanden.
    
    Erste Arbeiten von Gellersen und Schmidt\cite{webaware} brachten den Ansatz
    von \emph{Blicken in die Besucherseiten (Glances into visitors sites)}
    hervor. Die Annahme war da\"s die meisten Besucher \"uber einen eigenen
    Platz im Netz verf\"ugen und da"s ein Blick in die Pl\"atze der Besucher dem
    Gastgeber ein Verst\"andnis f\"ur das \emph{Territorium} vermittelt in dem
    er sich befindet.
    
    In der vorliegenden Arbeit werden wir diesen Ansatz fortentwickeln: Wir
    argumentiere da\"s selbst ein Besucher, der keine \emph{Heimat} im Netz
    hat (oder dessen Platz wir einfach nicht finden k"onnen) immer Verbindungen
    (oder Relationen) zu veschiedenen Pl\"atzen haben wird. Indem wir
    verschiedenen Arten von Relationen folgen, k"onnen wir verschiedene Arten
    von \emph{Terretorien} entdecken die in irgendeiner Weise an den Platz des
    Gastgebers angrenzen.
    
    Der Begriff der \emph{Relationen} zwischen Pl\"atzen im Netz ist nicht
    v"ollig neu, eine Reihe von Arbeiten hat sich bereits mit ihm besch"aftigt.
    (Unter anderem Adamic\cite{links}, Flake\cite{flake}, Gibson\cite{gibson}
    und Larson\cite{larson}). Adamic und Adar zeigten zum Beispiel, da\"s
    Hyperlinks soziale Gruppen erkennbar machen und da\"s diese Gruppen relativ
    identifiziert werden k\"onnen\cite{links}. Diese Ergebnisse zeigen, da\"s
    der hier verfolgte Ansatz, Relationen zu verfolgen, erfolgversprechend ist. 
